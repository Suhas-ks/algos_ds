    ...
    def check_bst(self, root):
        if root != None:
            ...
            if not (self.pseudo_root == root and root.parent == self.pseudo_root.parent):
                if self.in_order_count and root == root.parent.left and (root >= root.parent or (
                        not self.pseudo_flag and (
                        root >= self.pseudo_root or (root > self.in_order_min and root <= self.in_order_max)))):
                    return False
                elif self.in_order_count and root == root.parent.right and (root < root.parent or (
                        not self.pseudo_flag and (
                        (root > self.in_order_min and root < self.in_order_max) or root >= self.pseudo_root))):
                    return False
                elif self.in_order_count and root == root.parent.left and (root >= root.parent or (
                        self.pseudo_flag and (
                        (root > self.in_order_min and root <= self.in_order_max) or root < self.pseudo_root))):
                    return False
                elif self.in_order_count and root == root.parent.right and (root < root.parent or (
                        self.pseudo_flag and (
                    ...
                    if self.in_order_count == 1:
                        ...
                        self.in_order_min = self.in_order_max
                    elif root > self.in_order_max:
                        self.in_order_max = root
                    elif root < self.in_order_min:
                        self.in_order_min = root
            else:
                if self.in_order_count and ((root.left != None and root.left >= root) or (
                        root > self.in_order_min and root <= self.in_order_max) or (
